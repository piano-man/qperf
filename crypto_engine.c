                                ...
                                ptls_hash_algorithm_t *hash, const void *secret)
{
    ...
    DEBUG("GAGAN : AEAD algorithm name size when exporting secrets is %d\n", strlen(aead->name));
    ...
    //DEBUG("GAGAN : Secret when exporting secrets is %s\n", hexbuf);
    //DEBUG("GAGAN : Embedding name and length %s - %lu\n", embed_name, strlen(embed_name));
    //DEBUG("GAGAN : Embedding epoch and length %s - %lu\n", embed_epoch, strlen(embed_epoch));
...
static void default_finalize_send_packet(quicly_crypto_engine_t *engine, quicly_conn_t *conn,
                                         ...
                                         int coalesced)
{
    //DEBUG("GAGAN : Encryption function packet details\n");
    //DEBUG("GAGAN : Datagram length in encryption function %lu\n", datagram.len);
    //Ideally, first byte at should be 0 since we do not coalesce multipe quic packets into a udp datagram
    //DEBUG("GAGAN : First byte at %lu\n", first_byte_at);
    //payload from will indicate the point at which the quic payload starts
    //DEBUG("GAGAN : Payload from %lu\n", payload_from);
    //DEBUG("GAGAN : Packet number %lu\n", packet_number);
    //DEBUG("GAGAN : Dumping encrypted packet content to test things out\n"); //use quicly_hexdump
    //DEBUG("GAGAN : Printing packet epoch details %d\n", get_epoch(*(datagram.base+first_byte_at)));
    //DEBUG("GAGAN : Checking if packets are coalesced %d\n", coalesced);
    ...
    //DEBUG("GAGAN : Packet, header, and body len are %lu, %lu, %lu\n", packet_len, header_len, body_len);
    /*printf("\n\n\n\n");*/
